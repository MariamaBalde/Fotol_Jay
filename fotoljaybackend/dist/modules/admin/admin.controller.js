...
export class AdminController {
    ...
    async getUsers(req, res) {
        try {
            const { search = '', role = '', status = '', sortBy = 'date-desc', page = 1, limit = 20 } = req.query;
            ...
            // Logique de tri
            let orderBy = { dateCreation: 'desc' }; // Tri par défaut
            switch (sortBy) {
                case 'date-desc':
    ...
    async getReports(req, res) {
        try {
            ...
            const total = await prisma.signalement.count({ where: { statut: status } });
            res.json({
                reports,
                total,
                page: Number(page),
    ...
    async getModerators(req, res) {
        try {
            ...
            const moderatorsWithStats = await Promise.all(moderators.map(async (moderator) => {
    ...
    async getModeratorHistory(req, res) {
        try {
    ...
    async getModeratorRanking(req, res) {
        try {
            const moderators = await prisma.utilisateur.findMany({
                where: {
                    role: { in: ['MODERATEUR', 'ADMINISTRATEUR'] }
            ...
            const ranking = await Promise.all(moderators.map(async (moderator) => {
    ...
    async submitModeratorFeedback(req, res) {
        try {
            const { moderatorId } = req.params;
    ...
    async updateVipSetting(req, res) {
        try {
    ...
    async createVipPricing(req, res) {
        try {
    ...
    async updateVipPricing(req, res) {
        try {
            const { id } = req.params;
    ...
    // Codes promo VIP
    async getVipPromoCodes(req, res) {
        try {
    ...
    async createVipPromoCode(req, res) {
        try {
    ...
    async updateVipPromoCode(req, res) {
        try {
            const { id } = req.params;
    ...
    async getVipSubscriptions(req, res) {
        try {
            ...
            const total = await prisma.abonnementVIP.count({ where: { statut: status } });
            res.json({
                subscriptions,
                total,
                page: Number(page),
    ...
    async createVipSubscription(req, res) {
        try {
    ...
    async extendVipSubscription(req, res) {
        try {
    ...
    async refundVipSubscription(req, res) {
        try {
            const { subscriptionId } = req.params;
