...
export class ProduitsService {
    ...
    async creerProduit(donnees, utilisateurId, images) {
    ...
    async obtenirProduit(id, incrementerVue = true) {
    ...
    async obtenirMesProduits(utilisateurId, page = 1, limite = 20) {
    ...
    // Modifier un produit
    async modifierProduit(id, utilisateurId, donnees) {
        const produit = await this.repository.trouverParId(id);
        if (!produit) {
            throw new Error('Produit non trouvé');
        }
    ...
    async supprimerProduit(id, utilisateurId, role) {
        const produit = await this.repository.trouverParId(id);
        if (!produit) {
            throw new Error('Produit non trouvé');
        }
    ...
    async modererProduit(id, statut, raisonRefus) {
        const produit = await this.repository.trouverParId(id);
        if (!produit) {
            throw new Error('Produit non trouvé');
        }
        ...
        const produitModere = await this.repository.moderer(id, statut === 'APPROUVE' ? StatutProduit.APPROUVE : StatutProduit.REFUSE, raisonRefus);
        // Envoyer une notification à l'utilisateur
        if (statut === 'APPROUVE') {
            await this.serviceNotifications.notifierProduitApprouve(produit.utilisateurId, produit.titre, produit.id);
        }
        else {
            await this.serviceNotifications.notifierProduitRefuse(produit.utilisateurId, produit.titre, raisonRefus);
        }
    ...
    async enregistrerContact(id) {
        ...
        // Notifier le vendeur
        await this.serviceNotifications.notifierContactRecu(produit.utilisateurId, produit.titre, produit.id);
        return { message: 'Contact enregistré' };
    }
}
//# sourceMappingURL=ProduitsService.js.map