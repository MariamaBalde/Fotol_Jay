 ...
 */
export async function obtenirMesNotifications(
  req: AuthRequest,
  res: Response
): Promise<Response> {
  try {
    ...
    const resultat = await serviceNotifications.obtenirNotificationsUtilisateur(
      utilisateurId,
      { seulement_non_lues, page, limite }
    );
 ...
 */
export async function compterNonLues(req: AuthRequest, res: Response): Promise<Response> {
  try {
    const utilisateurId = req.user!.userId;
 ...
 */
export async function marquerCommeLue(req: AuthRequest, res: Response): Promise<Response> {
  try {
    const { id } = req.params;
    const utilisateurId = req.user!.userId;
